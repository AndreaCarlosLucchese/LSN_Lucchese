...
double Salesman_Path::ComputeFit(City cities){
        ...
        m_fitness+=cities.get_L1(m_path[m_path.size()-1], m_path[0]); //return to first city
    return m_fitness;
}
...
void SA_Algo::Simluated_Annealing(unsigned int nstep){ // Algoritmo di Simulated Annealing modificato per utilizzare la parallelizzazione
    double prob=0;
    double appo=m_T;
...
vector <double> SA_Algo::get_L(){
 ...
 void SA_Algo::ChangePath(vector <unsigned int> appo){
     path=appo;
 }
